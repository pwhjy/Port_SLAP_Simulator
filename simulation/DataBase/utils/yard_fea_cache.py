...
class Yard_cache(object):
    ...
    def slot_and_update(self, curcon, slot, existflag = (0,0)):
        ...
        self.blocks.setdefault(block, block_cache(self.vesselid, block,
                                                  self.yardconfig[block] if block in self.yardconfig else self.yardconfig["default"],
                                                  self.weightclass))
    ...
    def updata_reward(self, curcon, slot, tier_weight, stack_contype):
        ...
        self.block_Density[vessel]["block_Density"] = self.block_Density[vessel]["con_num"]/self.block_Density[vessel]["block_num"]
        # 场泊对应度
        ...
        self.concentration["sumconc"] += update_concentration(stack_contype, con_maxsum = self.blocks[block].maxtier, k1 = self.ck1, k2 = self.ck2)
        self.concentration["concentration"] = self.concentration["sumconc"] / self.stack_Equilibrium["sum_blocks_vesssel"]
...
class block_cache(object):
    def __init__(self, vesselid, block, capacity, weightclass = 12):
        ...
        self.stackpiling_rate = (self.stackpiling_20TEU["consum"] + 2 * self.stackpiling_40TEU["consum"]) / self.capacity # 56 堆存率
    ...
    def slot_and_update(self, curcon, slot, existflag):
        ...
        self.bays.setdefault(bay,bay_cache(self.vesselid, self.block, bay, self.stacknum, self.maxtier, self.weightclass))
        ...
        self.stackpiling_rate = (self.stackpiling_20TEU["consum"] + 2 * self.stackpiling_40TEU["consum"]) / self.capacity
